...
int lua_getcursor(lua_State* state) {
  ...
  lua_pushnumber(state, scrp->CursorGetRow());
  lua_pushnumber(state, scrp->CursorGetColumn());
  return 2;
}
...
int lua_setcursor(lua_State* state) {
  ...
  CKernel::Get()->mScreen.CursorMove(row, col);
  return 0;
}
...
int lua_conmode(lua_State* state) {
  ...
  if(args > 0) {
    if(lua_isinteger(state, 1)) {
      ...
      if(!strcmp(val, "sane")) {
	CKernel::Get()->mConsole.SetOptions(
				     CONSOLE_OPTION_ICANON|CONSOLE_OPTION_ECHO);
      }
    }
  }
  return 1;
...
extern "C"
int lua_charxy(lua_State* state) {
  const char* ch = lua_tostring(state, 1);
  lua_Integer px = luaL_checkinteger(state, 2);
  lua_Integer py = luaL_checkinteger(state, 3);
  lua_Integer col = luaL_checkinteger(state, 4);
  CKernel::Get()->mScreen.DisplayChar(*ch, px, py, col);
  return 0;
}

extern "C"
int lua_printxy(lua_State* state) {
  const char* ch = lua_tostring(state, 1);
  lua_Integer px = luaL_checkinteger(state, 2);
  lua_Integer py = luaL_checkinteger(state, 3);
  lua_Integer col = luaL_checkinteger(state, 4);
  while(*ch) {
    CKernel::Get()->mScreen.DisplayChar(*ch, px, py, col);
    px += CKernel::Get()->mScreen.GetColWidth();
    ch++;
  }
  return 0;
}
...
extern "C"
int luaopen_console(lua_State* state) {
