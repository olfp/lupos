...
extern "C"
FRESULT f_mopendir(DIR *dir, const char* path) {
  // try to opendir, if "not enabled" try to mount fs, retry opendir
  FRESULT Result = f_opendir(dir, path);
  if (Result == FR_NOT_ENABLED) {
    // try to mount volume 
    FATFS* pNewFs = &fatfs[f_getldn(path)];
    Result = f_mount(pNewFs, path, 1);
    if (Result == FR_OK) {
      // retry opendir
...
int lua_getdir(lua_State* state) {
  ...
  DIR dir;
  FILINFO info;
  FRESULT Result = f_mopendir(&dir, path);
  if (Result == FR_OK) {
    lua_newtable(state);
    while (true) {
...
int lua_chmod(lua_State* state) {
  if(!(lua_isstring(state, 1) && 
       lua_isinteger(state, 2) && lua_isinteger(state, 3))) {
    lua_pushnil(state);
...
extern "C"
int luaopen_filesystem(lua_State* state) {
