function dir(pathplus)
  ...
  if not dir then
    return "No such volume or path "..pathplus
  end
...
Options after ?: 
    ...
    for i, f in ipairs(names) do
      ...
      show = show and ((dironly and isdir) or (nodirs and not isdir) 
        or (not (dironly or nodirs))) -- no opt, show both
    ...
    if n < 1 then
      ...
      if dironly == nodirs or (dironly and nodirs) then out = out.."or " end -- xor 
      if not nodirs or dironly then out = out.."folders " end
      out = out.."found in "
      out = out..((#path > 0) and "'"..path.."' " or "current path ")
